...
- **HP Tuners VCM Suite (BETA)**: MG1CS036 direct OBDII flash — first-to-market, no PCM swap needed. This repo standardizes on MPVI4 workflows. 4 Universal Credits per PCM, TCM separate.
...
| `VCMScannerUserGuide.pdf` | HP Tuners VCM Scanner User Guide (Jan 2026, 6,600+ lines) — scanning/logging, channels, layouts (gauge/graph/chart/drive cycle), math parameters, RTT, standalone logging, vehicle profiles, controls |
| `MPVI4UserGuide.pdf` | HP Tuners MPVI4 User Guide (Feb 2026) — ports (OBD-II, HPTNET, USB-C), RGB LED status codes, BLE/WiFi, standalone logging button, ProLink+ wiring (analog + CAN), registration, TDN app |
...
| `hptuners-platform-overview.md` | HP Tuners platform: VCM Editor (table editor, compare tools, smoothing/interpolation, keyboard shortcuts, Paste Special, Ford tools), VCM Scanner (channels, layouts, math parameters, expression syntax, standalone logging, RTT, vehicle profiles, controls), MPVI3/4 hardware (LED status, ProLink+ wiring), credits/licensing, read/write workflow |
